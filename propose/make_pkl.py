...
# 引数:python make_pkl.py train　nomal 2048 960 480 16 48000
data_type = args[1] #trainか、testか
state = args[2] #nomalかabnomalか
...
def write_pkl(data, data_type, state):
    #例.  ./pickle/train/nomalのフォルダを作成
    commands.getoutput('mkdir -p ./pickle/%s/%s/' % (data_type, state))
    #例.  ./pickle/train/nomal/train_normal.pklを開く
...
if __name__ == '__main__':
    ...
    if os.path.exists('./pickle/train/nomal/train_nomal.pkl'):
        print('-----------------------------------')
    ...
    print('  state: {}'.format(state)) #nomalかabnomalか
    ...
    #配列iはファイル数、配列filePathは./sound/data_type(trainやtestなど)/$state(nomal等)/a.raw
    for i, filePath in enumerate(files): 
    
        sp = tool_propose.spectrum_envelope(filePath, N_FFT, frameLen, Shift, bit, frequency, i, data_type, state)
