...
def spectrum_envelope(filePath, N, frameLen, shift, bit, frequency, num, data_type, state):
    ...
    #1フレーム毎に処理
    for i in range(0, zx.shape[0]-11*shift, shift):
    #for i in range(0, x.shape[0]-shift, shift):
    
        #二重目のループ回数を数えるために初期化
        counter = 0
        
        #11フレーム毎に処理
        for p in range(i, 11*shift+i, shift):

            #ファイルの初めからフレーム数分、値を参照
            wave = zx[ p : p+frameLen]
            ...
            if i==0: # 初回のみコピー，その他は同じ変数にスタック
                ...
                if counter==0:
                    row = cutPdfLog
                    #print('    counter={0}   x[{1}~{2}]  row={3}'.format(counter, p, p+frameLen, row.shape) )
                elif counter==10:
                    row = np.append(row, cutPdfLog, axis=0)
                    #print('    counter={0}   x[{1}~{2}]  row={3}'.format(counter, p, p+frameLen, row.shape) )
                    sum_row = row.mean( axis = 0 )   #平均
                    #軸の変換を行う
                    sum_row = sum_row.reshape((1,sum_row.shape[0]))
                    output = sum_row
                ...
                if counter==0:
                    ...
                    #軸の変換を行う
                    sum_row = sum_row.reshape((1,sum_row.shape[0]))
                    output = np.append(output, sum_row, axis=0)
                    #print('  output={0}'.format(output.shape) )
                    del row
                    del sum_row
                else:
